...
'''열 한개에 여러 정보 있을 경우 2023-12-04'''
def create_checklist_2(sheet_name , input_file, output_file, criterion, required_parts):
    '''
    체크리스트생성
    '''
...
def create_checklist2(sheet_name, input_file, output_file, criterion, required_parts):
    '''
    체크리스트생성
    '''
...
if __name__ == "__main__":
    x='TEST'
    a=fr'D:\파이썬결과물저장소\CLM\source.xlsx'
    b=fr"D:\파이썬결과물저장소\CLM\result\TEST_{time.strftime('%y%m%d_%H%M%S')}.xlsx"
    c='제작 아이템'
    d=['수량','필요 재료','소모비용','제작 제한']


    create_checklist(x,a,b,c,d)
    #create_checklist2(x,a,b,c,d)